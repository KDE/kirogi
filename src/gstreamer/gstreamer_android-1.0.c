...
#define GST_G_IO_MODULE_DECLARE(name) extern void G_PASTE(g_io_module_, G_PASTE(name, _load_static))(void)

#define GST_G_IO_MODULE_LOAD(name) G_PASTE(g_io_module_, G_PASTE(name, _load_static))()
...
void glib_log_handler(const gchar *log_domain, GLogLevelFlags log_level, const gchar *message, gpointer user_data)
{
    ...
    if (((log_level & INFO_LEVELS) == 0) || domains == NULL || (strcmp(domains, "all") != 0 && (!log_domain || !strstr(domains, log_domain))))
        return;
...
void gst_debug_logcat(GstDebugCategory *category, GstDebugLevel level, const gchar *file, const gchar *function, gint line, GObject *object, GstDebugMessage *message, gpointer unused)
{
...
static gboolean get_application_dirs(JNIEnv *env, jobject context, gchar **cache_dir, gchar **files_dir)
{
    ...
    *cache_dir = *files_dir = NULL;
    ...
    get_cache_dir_id = (*env)->GetMethodID(env, context_class, "getCacheDir", "()Ljava/io/File;");
    get_files_dir_id = (*env)->GetMethodID(env, context_class, "getFilesDir", "()Ljava/io/File;");
    ...
    get_absolute_path_id = (*env)->GetMethodID(env, file_class, "getAbsolutePath", "()Ljava/lang/String;");
...
{
    __android_log_print(ANDROID_LOG_ERROR, "GStreamer", "get_application_context %p", _context);
    return _context;
}

jobject gst_android_get_application_class_loader(void)
{
    __android_log_print(ANDROID_LOG_ERROR, "GStreamer", "get_application_class_loader %p", _class_loader);
    return _class_loader;
}

JavaVM *gst_android_get_java_vm(void)
{
    __android_log_print(ANDROID_LOG_ERROR, "GStreamer", "get_java_vm %p", _java_vm);
    return _java_vm;
}
...
void gst_android_init(JNIEnv *env, jclass klass, jobject context)
{
    ...
    if (gst_is_initialized()) {
        __android_log_print(ANDROID_LOG_INFO, "GStreamer", "GStreamer already initialized");
        return;
    }

    if (!get_application_dirs(env, context, &cache_dir, &files_dir)) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamer", "Failed to get application dirs");
    }
    ...
    __android_log_print(ANDROID_LOG_INFO, "GStreamer", "GStreamer initialization complete");
}
#if 0
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    __android_log_print(ANDROID_LOG_ERROR, "GStreamer", "JNI_OnLoad");
