...
public:
    enum Type { Bebop2 = 0, Anafi };
    Q_ENUM(Type)

    explicit ParrotVehicle(Type type, const QString &hostName, int port, const QString &productSerial, QObject *parent = nullptr);
...
private:
    void initVehicle();
    void sendCommand(Parrot::Command command, const QVariantList &arguments = QVariantList(), bool retryForever = false);
