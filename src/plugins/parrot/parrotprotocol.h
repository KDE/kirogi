    ...
    public:
        struct Tuple {
            inline explicit Tuple(quint8 _productId = 0, quint8 _classId = 0, quint16 _commandId = 0)
                : productId(_productId), classId(_classId), commandId(_commandId) {}

            quint8 productId;
            quint8 classId;
            quint16 commandId;

            inline bool operator==(const Tuple &t) const { return (productId == t.productId
                && classId == t.classId && commandId == t.commandId); }
        };
...
inline uint qHash(const ParrotCommand::Tuple &t) { return t.productId * t.classId * t.commandId; }

class ParrotFrame {
    public:
        explicit ParrotFrame(quint8 dataType, quint8 bufferId, quint8 seq,
